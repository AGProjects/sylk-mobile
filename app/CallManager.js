...
export default class CallManager extends events.EventEmitter {
	...
	_rnAnswer(data) {
		...
		if (this._sessions.has(data.callUUID.toLowerCase())) {
			...
			this._waitingCalls.set(data.callUUID.toLowerCase(), 'answer');
		}
	}
	...
	_rnEnd(data) {
		...
		if (this._sessions.has(data.callUUID.toLowerCase())) {
			...
			this._waitingCalls.set(data.callUUID.toLowerCase(), 'decline');
		}
	}
	...
	handleSession(session, sessionUUID) {
		...
		session._callkeepUUID = sessionUUID || uuid.v4();
