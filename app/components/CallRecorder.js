...
const CallRecorder = {
    ...
    async stop() {
        ...
        // In-line spectrogram (remote leg) computed natively on the same
        // 100 ms bins as the peaks — already in spectrumCodec.js metadata
        // shape. Parse to an object so callers can drop it straight into
        // message metadata. Absent on older bridges (falls back to the JS
        // analyser in AudioCallBox).
        let spectrum = null;
        if (result && typeof result.spectrum === 'string') {
            try {
                const s = JSON.parse(result.spectrum);
                if (s && s.data && s.count > 0) spectrum = s;
            } catch (e) {
                spectrum = null;
            }
        }
        return {
            ...
            peaks: peaks,
            spectrum: spectrum,
            // Exact encoded-audio duration (ms) from the native presentation
            // clock. 0/undefined on older bridges.
